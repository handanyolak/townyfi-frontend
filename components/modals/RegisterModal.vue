<template>
  <div class="flex flex-col items-center space-y-4 p-12">
    ...
    <ListItem class="w-full" input>
      ...
      <template #item>
        <VForm class="flex flex-col items-center">
          <VField v-model="referrer" name="referrer" :rules="referrer && referrerRules" />
          <VErrorMessage class="text-red-800" name="referrer" />
        </VForm>
      </template>
    </ListItem>
    <AppButton v-if="ktaAllowance <= BigInt(setting?.price.register ?? 0)" class="my-3" @click="userApprove()">Approve
    </AppButton>
    <AppButton v-else class="my-3" @click="userRegister()">Register</AppButton>
    <AppButton class="my-3" @click="addKtaTokenToWallet()">Add Token to Metamask</AppButton>
  </div>
</template>
...
const { getKtaToken, getKtaCaller, getKtaTokenCaller } = storeToRefs(connectionStore)
...
const userRegister = async () => {
  const result = await getKtaCaller.value.callFunction('register', [
...
const userApprove = async () => {
  ...
  if (ktaTokenBalance < BigInt(setting.value?.price.register ?? 0)) {
    useAppToast(TYPE.ERROR, `You don't have enough tokens (${setting.value?.price.register}) to register for the game`)
    return
  }
  await getKtaTokenCaller.value.callFunction('approve', [ktaAddress, ktaTokenBalance], false)
}
